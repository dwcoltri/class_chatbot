...
def get_personas():
    return jsonify(
        {persona_id: {"name": data["name"]} for persona_id, data in PERSONAS.items()}
    )
    ...
    try:
        ...
        if response.candidates and response.candidates[0].content.parts:
            ...
            if finish_reason == 2:  # MAX_TOKENS
                ...
                assistant_message = (
                    "I apologize, but I can't respond to that due to safety guidelines."
                )
            else:
                assistant_message = (
        ...
        return jsonify({"message": assistant_message, "persona": persona_id})
